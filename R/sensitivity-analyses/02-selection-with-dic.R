###-------------------------------------------------------------
###   SENSITIVITY-02: COVARIATE SELECTION WITH DIC
###------------------------------------------------------------

# ---------------------------------------------------------------------------------------------------------------------------------------------------------
# 
# In this script, we fit the conditional autoregressive models in INLA (with the same specification as in
# "R/04-primary-analysis-inla.R") but we use the Watanabe–Akaike information criterion (WAIC) instead of 
# the Deviance Information Criterion (DIC). We also use a threshold difference of "5" for the WAIC for 
# decisions on keeping/removing variables.
# 
# 
#
...
# jhu
dic.res.jhu.1 <- car_inla_analysis( d.jhu = d.poly.jhu, d.cdc = d.poly.cdc,
...
# cdc
dic.res.cdc.1 <- car_inla_analysis( d.jhu = d.poly.jhu, d.cdc = d.poly.cdc,
                                formula.jhu = f.jhu, formula.cdc = f.cdc,
...
## (3.2) Save Results ##
 ...
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/01-main-analysis" )
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/01-main-analysis/jhu-results" )
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/01-main-analysis/cdc-results" )

inla_results_save( dic.res.jhu.1, path = paste0( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/01-main-analysis/jhu-results/" ),
                   tag = "waic-jhu-main", map.decomp.height = 2060  )

inla_results_save( dic.res.cdc.1, path = paste0( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/01-main-analysis/cdc-results/" ),
                   tag = "waic-cdc-main", map.decomp.height = 2060  )
...
# jhu
dic.res.jhu.ne <- car_inla_analysis( d.jhu = d.poly.jhu.ne, d.cdc = d.poly.cdc.ne,
                                 formula.jhu = f.jhu, formula.cdc = f.cdc, 
...
# cdc
dic.res.cdc.ne <- car_inla_analysis( d.jhu = d.poly.jhu.ne, d.cdc = d.poly.cdc.ne,
                                 formula.jhu = f.jhu, formula.cdc = f.cdc, 
...
# dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/02-northeast-analysis" )
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/02-northeast-analysis/jhu-results" )
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/02-northeast-analysis/cdc-results" )

inla_results_save( dic.res.jhu.ne, path = paste0( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/02-northeast-analysis/jhu-results/" ),
                   tag = "waic-jhu-ne", map.decomp.width = (4124)*0.7, map.decomp.height = (3808)*0.7 )

inla_results_save( dic.res.cdc.ne, path = paste0( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/02-northeast-analysis/cdc-results/" ),
                   tag = "waic-cdc-ne", map.decomp.width = (4124)*0.7, map.decomp.height = (3808)*0.7 )
...
# jhu
dic.res.jhu.so <- car_inla_analysis( d.jhu = d.poly.jhu.so, d.cdc = d.poly.cdc.so,
                                 formula.jhu = f.jhu, formula.cdc = f.cdc, 
                                 term = "fi", which.model = "jhu", E = "E_d.j", 
                                 null.model.formula = f.jhu.model.null,
                                 model.2.formula = f.jhu.model.2, 
                                 ...
                                 x.legend.pos = 0.78 )
# cdc
dic.res.cdc.so <- car_inla_analysis( d.jhu = d.poly.jhu.so, d.cdc = d.poly.cdc.so,
                                 formula.jhu = f.jhu, formula.cdc = f.cdc, 
                                 term = "fi", which.model = "cdc", E = "E_d.c", 
                                 null.model.formula = f.cdc.model.null,
                                 model.2.formula = f.cdc.model.2, 
...
# dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/03-south-analysis" )
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/03-south-analysis/jhu-results" )
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/03-south-analysis/cdc-results" )

inla_results_save( dic.res.jhu.so, path = paste0( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/03-south-analysis/jhu-results/" ),
                   tag = "waic-jhu-so" )

inla_results_save( dic.res.cdc.so, path = paste0( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/03-south-analysis/cdc-results/" ),
                   tag = "waic-cdc-so" )
...
# jhu
dic.res.jhu.mw <- car_inla_analysis( d.jhu = d.poly.jhu.mw, d.cdc = d.poly.cdc.mw,
                                 formula.jhu = f.jhu, formula.cdc = f.cdc, 
                                 term = "fi", which.model = "jhu", E = "E_d.j", 
                                 null.model.formula = f.jhu.model.null,
                                 model.2.formula = f.jhu.model.2, 
...
dic.res.cdc.mw <- car_inla_analysis( d.jhu = d.poly.jhu.mw, d.cdc = d.poly.cdc.mw,
                                 formula.jhu = f.jhu, formula.cdc = f.cdc, 
                                 term = "fi", which.model = "cdc", E = "E_d.c", 
                                 null.model.formula = f.cdc.model.null,
                                 model.2.formula = f.cdc.model.2, 
...
# dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/04-midwest-analysis" )
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/04-midwest-analysis/jhu-results" )
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/04-midwest-analysis/cdc-results" )

inla_results_save( dic.res.jhu.mw, path = paste0( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/04-midwest-analysis/jhu-results/" ),
                   tag = "waic-jhu-mw" )

inla_results_save( dic.res.cdc.mw, path = paste0( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/04-midwest-analysis/cdc-results/" ),
                   tag = "waic-cdc-mw" )
...
# jhu
dic.res.jhu.w <- car_inla_analysis( d.jhu = d.poly.jhu.w, d.cdc = d.poly.cdc.w,
                                formula.jhu = f.jhu, formula.cdc = f.cdc, 
                                term = "fi", which.model = "jhu", E = "E_d.j", 
                                null.model.formula = f.jhu.model.null,
                                model.2.formula = f.jhu.model.2, 
                                ...
                                x.legend.pos = 0.78 )
# cdc
dic.res.cdc.w <- car_inla_analysis( d.jhu = d.poly.jhu.w, d.cdc = d.poly.cdc.w,
                                formula.jhu = f.jhu, formula.cdc = f.cdc, 
                                term = "fi", which.model = "cdc", E = "E_d.c", 
                                null.model.formula = f.cdc.model.null,
                                model.2.formula = f.cdc.model.2, 
...
## (6.3) Save Results ##
 ...
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/05-west-analysis" )
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/05-west-analysis/jhu-results" )
 # dir.create( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/05-west-analysis/cdc-results" )

inla_results_save( dic.res.jhu.w, path = paste0( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/05-west-analysis/jhu-results/" ),
                   tag = "waic-jhu-w" )

inla_results_save( dic.res.cdc.w, path = paste0( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/05-west-analysis/cdc-results/" ),
                   tag = "waic-cdc-w" )
...
list.mods.jhu <- list(  dic.res.jhu.ne$model.fit,
                        dic.res.jhu.so$model.fit,
                        dic.res.jhu.mw$model.fit,
                        dic.res.jhu.w$model.fit,
                        dic.res.jhu.1$model.fit )

list.mods.cdc <- list(  dic.res.cdc.ne$model.fit,
                        dic.res.cdc.so$model.fit,
                        dic.res.cdc.mw$model.fit,
                        dic.res.cdc.w$model.fit,
                        dic.res.cdc.1$model.fit )
...
ggsave( "04-Tables-Figures/06-sensitivity-analyses/03-selection-with-dic/dic-forest-plot-both-all-analyses.png",
        width = 6, height = 8 )
